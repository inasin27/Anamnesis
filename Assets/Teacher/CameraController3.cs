...
public class CameraController3 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //this.transform.position = new Vector3( this.player.transform.position.x + (Mathf.Sin( Time.time) * radius), this.transform.position.y, this.player.transform.position.z + (Mathf.Cos( Time.time) * radius));
		...
		//注視点（プレイヤー）
		//transform.LookAt( this.player.transform.position + new Vector3( 0f, 1f, 0f));
